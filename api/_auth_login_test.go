...
func TestServerLogin(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() {
				// Mocking successful user retrieval
		...
		},
		{
			...
			setup: func() {
				// Mocking user retrieval with incorrect password
				mock.ExpectQuery("^SELECT (.+) FROM users WHERE username=?").
					WithArgs("validuser").
					WillReturnRows(sqlmock.NewRows([]string{"id", "username", "password", "email", "created_at"}).
						AddRow(1, "validuser", "$2a$10$H8iJ0", "email@example.com", time.Now()))
		...
		},
		{
			...
			setup: func() {
				// Mocking user retrieval with malformed hash
				mock.ExpectQuery("^SELECT (.+) FROM users WHERE username=?").
					WithArgs("validuser").
					WillReturnRows(sqlmock.NewRows([]string{"id", "username", "password", "email", "created_at"}).
		...
		},
		{
			...
			setup: func() {
				// No setup needed for JSON error
			},
			requestBody:    `{"username": "validuser", "password": "correctpass"}`, // Fixed JSON format
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Additional logging
			t.Logf("Test case '%s' executed with status %d", tt.name, recorder.Code)
		})
	}
}
